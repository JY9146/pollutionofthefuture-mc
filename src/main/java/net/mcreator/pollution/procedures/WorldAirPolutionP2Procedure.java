...
public class WorldAirPolutionP2Procedure {
	...
	private static void buildClouds(CloudStatus cloudStatus, double x, double y, double z) {
		if (cloudBuffer == null || WorldAirPolutionP2Procedure.cloudStatus != cloudStatus || WorldAirPolutionP2Procedure.x != x || WorldAirPolutionP2Procedure.y != y || WorldAirPolutionP2Procedure.z != z) {
			WorldAirPolutionP2Procedure.cloudStatus = cloudStatus;
			WorldAirPolutionP2Procedure.x = x;
			WorldAirPolutionP2Procedure.y = y;
			WorldAirPolutionP2Procedure.z = z;
			Minecraft minecraft = Minecraft.getInstance();
	...
	private static void execute(@Nullable Event event, LevelAccessor world, ResourceKey<Level> dimension) {
		...
		if (dimension == Level.OVERWORLD && world.getLevelData().getGameRules().getBoolean(PollutionModModGameRules.POLLUTED_AIR)) {
			...
			renderClouds(Minecraft.getInstance().options.getCloudsType(), 152, -1, 0, 255 << 24 | 63 << 16 | 69 << 8 | 81);
		}
	}
}
